...
export default function FormPage({ auth, item }: PageProps<{ item?: Order }>) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        },
        {
            ...
            href: item ? route('orders.edit', item.id) : route('orders.create'),
        },
    ];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={(item ? 'Edit' : 'Create') + ' Order'} />

            <div className="p-4 sm:p-6 lg:p-8">
                <form onSubmit={handleSubmit}>
                    <Card>
                        <CardHeader>
                            <CardTitle>{item ? 'Edit' : 'Create'} Order</CardTitle>
                        </CardHeader>
                        ...
                        <CardFooter className="flex justify-end gap-2">
                            <Link href={route('orders.index')} className={cn(buttonVariants({ variant: 'ghost' }))}>Cancel</Link>
